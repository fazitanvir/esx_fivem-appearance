...
-- Added for Dolu edits
ESX.RegisterServerCallback('fivem-appearance:BuyOutfit', function(source, cb, totalPrice, appearance, boughtCloths)
	local xPlayer = ESX.GetPlayerFromId(source)
	local playerMoney = xPlayer.getMoney()

	if playerMoney >= totalPrice then
		
		if boughtCloths ~= {} then
			for itemName, itemData in pairs(boughtCloths) do
				xPlayer.addInventoryItem(itemName, 1, {model = itemData.model, color = itemData.color})
			end
		end
		...
		xPlayer.showNotification('You dont have enough money, missing $'..(totalPrice - playerMoney))
		cb(false)
	end
end)
--
